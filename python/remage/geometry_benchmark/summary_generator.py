...
def extract_geometry_edges_2d(gdml_file, tolerance=0.1):
    """
    Extract 2D silhouette/contour edges from GDML geometry for overlay on time plots.
    Only extracts boundary edges visible in each projection, suppressing internal edges.
    ...
    tolerance : float
        Tolerance for determining if an edge is perpendicular to view direction (mm)

    Returns:
    --------
    dict : Dictionary with 'xy', 'xz', 'yz' keys containing edge coordinates for each plane
    """
    # Read GDML geometry
    r = pyg4ometry.gdml.Reader(gdml_file)
    l = r.getRegistry().getWorldVolume()

    # Create VTK viewer to get mesh data
    v = pyg4ometry.visualisation.VtkViewerColouredMaterial()
    v.addLogicalVolume(l)

    # Dictionary to store edge projections for each plane
    edges_2d = {"xy": [], "xz": [], "yz": []}

    # Extract edges from all actors with their transformations
    for actor in v.actors:
        ...
        # Get the actor's transformation matrix (world coordinates)
        ...
        # Use vtkFeatureEdges to extract boundary and feature edges
        # This gives us silhouette edges rather than all edges
        featureEdges = vtk.vtkFeatureEdges()
        featureEdges.SetInputData(transformedPolydata)
        featureEdges.BoundaryEdgesOn()
        featureEdges.FeatureEdgesOn()
        featureEdges.ManifoldEdgesOff()
        featureEdges.NonManifoldEdgesOff()
        featureEdges.SetFeatureAngle(30)  # Edges with > 30 degree angle
        featureEdges.Update()

        edgePolyData = featureEdges.GetOutput()
        points = edgePolyData.GetPoints()

        if points is None:
            continue

        # Get lines (edges)
        lines = edgePolyData.GetLines()
        lines.InitTraversal()
        ...
        while lines.GetNextCell(idList):
            ...
            if idList.GetNumberOfIds() >= 2:
                p1 = np.array(points.GetPoint(idList.GetId(0)))
                p2 = np.array(points.GetPoint(idList.GetId(1)))
                ...
                # Store edge projections only if they have significant extent in the plane
                # XY plane (z=constant projection) - skip if edge is purely in Z direction
                proj_xy_length = np.linalg.norm([edge_vec[0], edge_vec[1]])
                if proj_xy_length > tolerance:
                    edges_2d["xy"].append([(p1[0], p1[1]), (p2[0], p2[1])])

                # XZ plane (y=constant projection) - skip if edge is purely in Y direction
                proj_xz_length = np.linalg.norm([edge_vec[0], edge_vec[2]])
                if proj_xz_length > tolerance:
                    edges_2d["xz"].append([(p1[0], p1[2]), (p2[0], p2[2])])

                # YZ plane (x=constant projection) - skip if edge is purely in X direction
                proj_yz_length = np.linalg.norm([edge_vec[1], edge_vec[2]])
                if proj_yz_length > tolerance:
                    edges_2d["yz"].append([(p1[1], p1[2]), (p2[1], p2[2])])

    return edges_2d


def plot_geometry_wireframe_on_ax(
    ax, edges, plane="xy", color="black", linewidth=0.5, alpha=0.3
):
    """
    Plot geometry edges as wireframe on a matplotlib axis.
    ...
    if len(edges) > 0:
        lc = LineCollection(edges, colors=color, linewidths=linewidth, alpha=alpha)
...
class SummaryGenerator:
    ...
    def __init__(self, sim_output_file, args):
        ...
        self.draw_wireframe = args.draw_wireframe
    ...
    def draw_simulation_time_profiles(self, suffix="simulation_time_profiles.pdf"):
        ...
        def draw(self, norm):
            ...
            # Overlay geometry wireframes if GDML file provided
            if self.draw_wireframe:
                try:
                    edges_2d = extract_geometry_edges_2d(self.gdml_file)
                    plot_geometry_wireframe_on_ax(
                        ax[0],
                        edges_2d["xy"],
                        plane="xy",
                        color="white",
                    ...
                    print(f"Warning: Could not overlay geometry wireframe: {e}")
