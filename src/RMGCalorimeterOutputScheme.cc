...
void RMGCalorimeterOutputScheme::AssignOutputNames(G4AnalysisManager* ana_man) {
  ...
  for (auto&& det : detectors) {
    if (det.second.type != RMGDetectorType::kCalorimeter) continue;
    ...
    if (volumes.empty()) {
      RMGLog::Out(
          RMGLog::fatal,
          "Could not find detector physical volume in RMGCalorimeterOutputScheme::AssignOutputNames"
      );
    }
    if (volumes.size() > 1) {
      RMGLog::Out(
...
RMGDetectorHitsCollection* RMGCalorimeterOutputScheme::GetHitColl(const G4Event* event) {
  auto sd_man = G4SDManager::GetSDMpointer();

  auto hit_coll_id = sd_man->GetCollectionID("Calorimeter/Hits");
  if (hit_coll_id < 0) {
    RMGLog::OutDev(RMGLog::error, "Could not find hit collection Calorimeter/Hits");
    return nullptr;
  }
...
void RMGCalorimeterOutputScheme::StoreEvent(const G4Event* event) {
  ...
  if (rmg_man->IsPersistencyEnabled()) {
    ...
    for (auto hit : *hit_coll->GetVector()) {
      ...
      ana_man->FillNtupleDColumn(ana_man, ntupleid, col_id++, hit->energy_deposition / u::keV);
