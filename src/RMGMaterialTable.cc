...
std::map<RMGMaterialTable::BathMaterial, RMGMaterialTable::PropertiesAtTemperature>
    RMGMaterialTable::fPropertiesAtTemperatureTable = {};
...
RMGMaterialTable::RMGMaterialTable() {
  ...
  fPropertiesAtTemperatureTable.emplace(BathMaterial::kAir,
      PropertiesAtTemperature("Air", 5.32 * u::g / u::cm3, 5.54 * u::g / u::cm3));

  fPropertiesAtTemperatureTable.emplace(BathMaterial::kLiquidArgon,
      PropertiesAtTemperature("LiquidArgon", 5.34 * u::g / u::cm3, 5.56 * u::g / u::cm3));
...
void RMGMaterialTable::InitializeMaterials() {
  ...
  fMaterialAliases = {{"Air", "G4_AIR"}, {"Brass", "G4_BRASS"}, {"Bronze", "G4_BRONZE"},
      {"Concrete", "G4_CONCRETE"}, {"Germanium", "G4_Ge"}, // with natural isotopic composition
      {"Kapton", "G4_KAPTON"}, {"LiquidArgon", "G4_lAr"}, {"LiquidNitrogen", "G4_lN2"},
      {"LiquidXenon", "G4_lXe"}, {"Nylon", "G4_NYLON-6-6"}, {"Teflon", "G4_TEFLON"},
      {"StainlessSteel", "G4_STAINLESS-STEEL"}, {"Vacuum", "G4_Galactic"}, {"Water", "G4_WATER"}};
  ...
  auto mat_enr_ge = new G4Material("RMG_EnrichedGermanium", density = 5.54, n_components = 1,
      state = G4State::kStateSolid, temperature = u::STP_Temperature, pressure = u::STP_Pressure);
  ...
  man->BuildMaterialWithNewDensity("RMG_CryogenicGermanium", "G4_Ge",
  ...
  // new germanium with cryogenic properties
  man->BuildMaterialWithNewDensity("RMG_CryogenicEnrichedGermanium", "RMG_EnrichedGermanium",
      density = fPropertiesAtTemperatureTable[bath_material].enriched_germanium_density,
      RMGMaterialTable::GetMaterial(bath_material)->GetTemperature(),
      RMGMaterialTable::GetMaterial(bath_material)->GetPressure());
}

void RMGMaterialTable::InitializeOpticalProperties() { this->InitializeLArOpticalProperties(); }
...
void RMGMaterialTable::InitializeLArOpticalProperties() {
  ...
  auto lar_dielectric_const = [](double lambda) {
    ...
    eps = 1.2055e-2 * (0.2075 / (91.012 - eps) + 0.0415 / (87.892 - eps) + 4.3330 / (214.02 - eps));
  ...
  auto lar_scint_spectrum = [](double kk) {
    return std::exp(
        -0.5 * ((kk - 128 * u::nm) / (2.929 * u::nm)) * ((kk - 128 * u::nm) / (2.929 * u::nm)));
  };
  ...
  RMGLog::OutFormat(RMGLog::debug, "Sampling scintillation spectrum from {:.3f} nm ({:.3f} eV) to {:.3f} nm ({:.3f} eV) with a {:.3f} eV step",
      to_wavelength(E1) / u::nm, E1 / u::eV, to_wavelength(E2) / u::nm, E2 / u::eV, dE / u::eV);
  ...
  RMGLog::Out(RMGLog::detail, "Using LAr singlet lifetime of ",
      fLArProperties.singlet_lifetime / u::ns, " ns");
  RMGLog::Out(RMGLog::detail, "Using LAr triplet lifetime of ",
      fLArProperties.triplet_lifetime / u::us, " us");
  ...
  RMGLog::OutFormat(RMGLog::debug, "Sampling LAr attenuation length from {:.3f} nm ({:.3f} eV) to {:.3f} nm ({:.3f} eV) with a {:.3f} eV step",
      to_wavelength(E1) / u::nm, E1 / u::eV, to_wavelength(E2) / u::nm, E2 / u::eV, dE / u::eV);
  ...
  for (double e = E1; e <= E2; e += dE) {
    ...
    lar_mpt->AddEntry("RAYLEIGH", e,
        lar_rayleigh_length(to_wavelength(e),
            RMGMaterialTable::GetMaterial("LiquidArgon")->GetTemperature()));

    // normalize absorption length
    lar_mpt->AddEntry("ABSLENGTH", e,
        lar_absorption_length(to_wavelength(e)) * fLArProperties.vuv_absorption_length /
            lar_absorption_length(128 * u::nm));
  }
  ...
  RMGMaterialTable::GetMaterial("LiquidArgon")
      ->GetIonisation()
      ->SetBirksConstant(5.1748e-4 * u::cm / u::MeV);
}
...
void RMGMaterialTable::DefineCommands() {
  ...
  fLArMessenger
      ->DeclarePropertyWithUnit("FlatTopPhotonYield", "1/keV", fLArProperties.flat_top_photon_yield)
  ...
  fLArMessenger
      ->DeclarePropertyWithUnit("VUVAbsorptionLength", "cm", fLArProperties.vuv_absorption_length)
