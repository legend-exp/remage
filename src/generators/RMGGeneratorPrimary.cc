...
#include "RMGGeneratorPositionSampling.hh"
...
RMGGeneratorPrimary::RMGGeneratorPrimary():
  ...
  fPositionSampler = new RMGGeneratorPositionSampling();
...
void RMGGeneratorPrimary::GeneratePrimaries(G4Event* event) {
  ...
  if (fRMGGenerator) {
    if (fConfinementCode == volume) {
      ...
      fRMGGenerator->SetParticlePosition(fPosition);
    }
    else if (fConfinementCode == volumelist) {
      if (this->GetVolumeListName() == "nullptr") {
        RMGLog::Out(RMGLog::fatal, "Volume name list is not defined, please set it using the proper macro command");
      }

      G4String volume_name = this->ChooseVolumeFromList();
      RMGLog::Out(RMGLog::debug, "Volume to be sampled from: ", volume_name);
...
void RMGGeneratorPrimary::SetConfinementCode(EConfinementCode code) {
  ...
  if (fConfinementCode == noconfined) {
    ...
    fRMGGenerator->SetParticlePosition(fPosition);
    RMGLog::Out(RMGLog::detail, "Default position re-set to ", fPosition);
  }
}

void RMGGeneratorPrimary::InitializeVolumeArraySampling() {

  RMGLog::Out(RMGLog::summary, "Start InitializeVolumeArraySampling");
  ...
  if (fIDVolumeArray.size() == 0) {
    RMGLog::Out(RMGLog::error, "Problem with the volumearray: no list of volumes selected ",
  ...
  for (size_t i = 0; i < fIDVolumeArray.size(); i++) {
    ...
    if (!phys_vol) {
      RMGLog::OutFormat(RMGLog::error, "The '%s' volume does not exist", volume_name.c_str());
      return;
    }

    G4double vol_mass = phys_vol->GetLogicalVolume()->GetMass(false, false);
    RMGLog::OutFormat(RMGLog::summary, "Mass of volume '%s' is %g", volume_name.c_str(), vol_mass/CLHEP::kg);
    tot_mass += vol_mass;
    fMassFractionForVolumeArray.push_back(tot_mass);
  }

  RMGLog::Out(RMGLog::summary, "Number of volumes: ", fMassFractionForVolumeArray.size()-1);
  RMGLog::Out(RMGLog::summary, "Total mass: ", tot_mass/CLHEP::kg);
  ...
  for (size_t k = 0; k < fMassFractionForVolumeArray.size(); k++) {
    ...
    RMGLog::OutFormat(RMGLog::summary, "Fractional mass nr. %i is %g", k, fMassFractionForVolumeArray.at(k));
  }

  fVolumeArrayInitialized = true;
}

G4String RMGGeneratorPrimary::ChooseVolumeFromList() {
  ...
  if (!fVolumeListInitialized) {
    ...
    if (!fVolumeListInitialized) { // if it is still false, it means that the initialization failed
      RMGLog::Out(RMGLog::fatal, "Not able to initialize the volume list");
    }
  }
  ...
  RMGLog::Out(RMGLog::debug, "Found volume nr. ", ifound, ", random number: ", ran);
  G4int element_number = -1;

  if (ifound > fIDVolumeList.size()-1) {
    RMGLog::Out(RMGLog::error, "Index of found volume past list size, sampling from the last volume in the list.");
...
G4String RMGGeneratorPrimary::ChooseVolumeFromArray() {
  ...
  if (!fVolumeArrayInitialized) {
      ...
      if (!fVolumeArrayInitialized) { // if it is still false, it means that the initialization failed
        RMGLog::Out(RMGLog::fatal, "Not able to initialize the array");
      }
  }
...
void RMGGeneratorPrimary::AddVolumeNumberToList(G4int det) {
  fIDVolumeList.push_back(det);
  RMGLog::Out(RMGLog::debug, "Added detector nr. ", det);
  RMGLog::Out(RMGLog::debug, "Size of the list: ", fIDVolumeList.size());
}

void RMGGeneratorPrimary::AddVolumeNameToArray(G4String det) {
  fIDVolumeArray.push_back(det);
  RMGLog::Out(RMGLog::debug, "Added detector nr. ", det);
  RMGLog::Out(RMGLog::debug, "Size of the list: ", fIDVolumeArray.size());
}
