...
void RMGVertexConfinement::SampleableObject::RecalcMass(int z, int a) {
  ...
  if (z > 0) {
    ...
    for (size_t i = 0; i < mat->GetNumberOfElements(); i++) {
      ...
      for (size_t j = 0; j < elm->GetNumberOfIsotopes(); j++) {
        ...
        // NOTE: despite its name, G4Isotope::GetN() returns the number of nucleons, i.e. the
        // mass number A.
        if (iso->GetZ() == z && iso->GetN() == a) {
  ...
  switch (type) {
    ...
    case Type::kTruncatedGaussian: {
      ...
      RMGLog::Out(
          ...
          "The range [range_lo, range_hi] lies too far in the tail of the Gaussian "
          "defined by (mean, sigma); adjust the parameters so acceptance is feasible."
      );
      return range_lo;
    }
...
void RMGVertexConfinement::DepthProfile::Validate() const {
  switch (type) {
    case Type::kNone: return;

    case Type::kExponential:
      if (mean <= 0)
        RMGLog::Out(RMGLog::fatal, "Exponential depth profile requires a strictly positive Mean");
      return;

    case Type::kUniform:
      if (range_lo < 0) RMGLog::Out(RMGLog::fatal, "Uniform depth profile requires RangeLow >= 0");
      if (range_hi <= range_lo)
        RMGLog::Out(RMGLog::fatal, "Uniform depth profile requires RangeHigh > RangeLow");
      return;

    case Type::kTruncatedGaussian:
      if (sigma <= 0)
        RMGLog::Out(RMGLog::fatal, "TruncatedGaussian depth profile requires a strictly positive Sigma");
      if (range_lo < 0)
        RMGLog::Out(RMGLog::fatal, "TruncatedGaussian depth profile requires RangeLow >= 0");
      if (range_hi <= range_lo)
        RMGLog::Out(RMGLog::fatal, "TruncatedGaussian depth profile requires RangeHigh > RangeLow");
      return;
  }
}

void RMGVertexConfinement::SampleableObject::ApplyDepthProfile(
    G4ThreeVector& local_vertex,
    const G4VSolid* solid
) const {
  if (this->depth_profile.type == DepthProfile::Type::kNone) return;

  const G4ThreeVector normal = solid->SurfaceNormal(local_vertex);
  double depth = this->depth_profile.Sample();
...
std::vector<G4ThreeVector> RMGVertexConfinement::SampleableObject::GetIntersections(
    G4ThreeVector start,
    G4ThreeVector dir
) const {
  ...
  if ((not this->physical_volume) or (not this->physical_volume->GetLogicalVolume()->GetSolid()))
    RMGLog::OutDev(
        RMGLog::fatal,
        "Cannot find number of intersections for a SampleableObject ",
...
void RMGVertexConfinement::SampleableObject::GetDirection(G4ThreeVector& dir, G4ThreeVector& pos) const {

  if ((not this->physical_volume) or (not this->physical_volume->GetLogicalVolume()->GetSolid()))
    RMGLog::OutDev(
        RMGLog::fatal,
        "Cannot generate directions for a SampleableObject ",
        "where the physical volume is not set. This probably means you are trying to "
        "generically sample a geometrical volume which instead should be natively sampled"
    );
...
bool RMGVertexConfinement::SampleableObject::GenerateSurfacePoint(
  ...
  RMGLog::Out(
      RMGLog::error,
      "Exceeded maximum number of allowed iterations (",
      max_attempts,
...
bool RMGVertexConfinement::SampleableObject::Sample(
    G4ThreeVector& vertex,
    size_t max_attempts,
  ...
  if (this->native_sample) {
    ...
    if (this->surface_sample) this->ApplyDepthProfile(local_vertex, this->sampling_solid);
    ...
    this->ApplyDepthProfile(vertex, this->physical_volume->GetLogicalVolume()->GetSolid());
    ...
    if (force_containment_check && !this->IsInside(vertex)) {
      ...
      RMGLog::OutDev(
          ...
          vertex / CLHEP::cm,
          " cm (",
          magic_enum::enum_name<EInside>(vol_type),
          ")"
      );
    }
  } else {
...
void RMGVertexConfinement::SampleableObjectCollection::recalc_total(
  ...
  for (auto& v : this->data) {
    ...
    v.RecalcMass(mass_isotope_z, mass_isotope_a);
...
void RMGVertexConfinement::InitializePhysicalVolumes() {
  ...
  for (auto&& el : fPhysicalVolumes.data) {
    ...
    if (RMGGeneratorUtil::IsSampleable(solid_type)) {
      ...
      if (log_vol->GetNoDaughters() > 0) {
        ...
        if (fOnSurface)
          RMGLog::OutDev(
              RMGLog::summary,
              "Surface sampling is on the outside of the mother volume "
    ...
    // use bounding solid for all other cases
      ...
      RMGLog::OutDev(
          RMGLog::debug,
          "Is not sampleable natively, need a bounding box with ",
          "containment check"
      );
      el.native_sample = false;
      el.surface_sample = false;
      ...
      el.sampling_solid = new G4Box(
          el.physical_volume->GetName() + "/RMGVertexConfinement::fBoundingBox",
          bb_x,
          bb_y,
          bb_z
      );
    ...
    for (size_t i = 1; i < trees.size(); ++i) {
      ...
      new_obj_from_inspection.emplace_back(
          el.physical_volume,
          v.vol_global_rotation,
          v.vol_global_translation,
          el.sampling_solid,
          el.native_sample,
          el.surface_sample
      );
  ...
  fPhysicalVolumes.recalc_total(fWeightByMass, fWeightByMassIsotopeZ, fWeightByMassIsotopeA);
...
void RMGVertexConfinement::InitializeGeometricalVolumes(bool use_excluded_volumes) {
  ...
  for (const auto& d : volume_data) {
    ...
    if (solid) {
      ...
      RMGLog::OutFormat(
          RMGLog::error,
          "Geometrical solid '{}' not known! (Implement me?)",
  ...
  // calculate the total volume/surface/mass.
  volume_solids.recalc_total(fWeightByMass, fWeightByMassIsotopeZ, fWeightByMassIsotopeA);
...
bool RMGVertexConfinement::GenerateVertex(G4ThreeVector& vertex) {
  ...
  fVertexGenerationTime += std::chrono::duration_cast<std::chrono::nanoseconds>(
      time_sampling_end - time_sampling_start
  );
  ...
  switch (fSamplingMode) {
    case SamplingMode::kIntersectPhysicalWithGeometrical: {
      if (fGeomVolumeSolids.empty() or fPhysicalVolumes.empty()) {
        RMGLog::Out(
            RMGLog::fatal,
            "'IntersectPhysicalWithGeometrical' mode is set but ",
      ...
      while (calls <= RMGVVertexGenerator::fMaxAttempts) {
        ...
        if (fOnSurface) {
          ...
          if (fFirstSamplingVolumeType == VolumeType::kUnset)
            RMGLog::Out(
                RMGLog::fatal,
                "For surface sampling vertex confinment mode it is required ",
    ...
    case SamplingMode::kSubtractGeometrical: {
      if (fGeomVolumeSolids.empty() and fPhysicalVolumes.empty()) {
        RMGLog::Out(
            RMGLog::fatal,
            "'SubtractGeometrical' mode is set but ",
      ...
      if (fFirstSamplingVolumeType == VolumeType::kGeometrical and not has_geometrical)
        RMGLog::Out(RMGLog::fatal, "Cannot sample first from geometrical volumes when none are set.");
...
void RMGVertexConfinement::AddExcludedGeometricalVolumeString(std::string solid) {
  ...
  if (fSamplingMode != SamplingMode::kSubtractGeometrical) {
    RMGLog::Out(
        RMGLog::fatal,
        "Cannot set excluded geometrical regions in any other sampling mode",
...
RMGVertexConfinement::GenericGeometricalSolidData& RMGVertexConfinement::SafeBack(
  ...
  if (volume_data.empty()) {
    RMGLog::Out(
        RMGLog::fatal,
        "Must call /RMG/Generator/Confinement/Geometrical/",
        command_name,
...
void RMGVertexConfinement::EndOfRunAction(const G4Run* run) {
  ...
  RMGLog::OutFormat(
      RMGLog::summary,
      "Stats: average time to sample a vertex was {:.5f} us/event",
...
void RMGVertexConfinement::DefineCommands() {
  ...
  fMessengers.back()
      ...
      .SetGuidance("Weigh the different volumes by mass of the given isotope")
      .SetGuidance(
          "The isotope is specified by its atomic number Z (number of protons) and its "
          "mass number A (number of nucleons), e.g. '32 76' for Ge-76"
      )
      .SetParameterName(0, "Z", false, false)
      .SetParameterName(1, "A", false, false)
      .SetStates(G4State_PreInit, G4State_Idle)
      .SetToBeBroadcasted(true);
  ...
  fMessengers.back()
      ...
      .SetGuidance(
          "Set maximum number of intersections of a line with the surface. Note: can be "
  ...
  fMessengers.back()
      ->DeclareMethodWithUnit("StartingAngle", "deg", &RMGVertexConfinement::SetGeomCylinderStartingAngle)
      .SetGuidance("Set starting angle")
      .SetParameterName("A", false)
      .SetStates(G4State_PreInit, G4State_Idle);

  fMessengers.back()
      ->DeclareMethodWithUnit("SpanningAngle", "deg", &RMGVertexConfinement::SetGeomCylinderSpanningAngle)
