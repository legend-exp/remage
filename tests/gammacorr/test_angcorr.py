...
def simulate(Z, A, level, angcorr):
    ...
    cmds = macro_template.format(Z=Z, A=A, level=level, angcorr=angcorr).split("\n")
...
def test_plot_gammacorr():
    ...
    for Z, A, title in items:
        ...
        for angcorr in (True, False):
            ...
            gtrk = trk[
                (trk.particle == 22) & (trk.procid == processes["RadioactiveDecay"])
            ]
        ...
        ax.legend()
        fig.savefig(f"{Z}-{A}-gamma-angular-distribution.output.png")
