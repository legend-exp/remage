...
def _gamma_macro_volume(
    *,
    seed: int,
    events: int,
    stack_volume: str,
) -> str:
    """Gamma source producing secondary electrons in ``world_vol`` only, with
    electron staging restricted to ``stack_volume`` (safety 0, no energy
    threshold) to isolate the volume-name selection logic.

    The detector sits at the origin (a 2.5 cm radius, 5 cm tall Ge tube). To
    guarantee that *no* secondaries are created inside ``detector_vol``, the
    source is placed far away (z = 100 cm) and fires a directional pencil beam
    in +x, i.e. in the plane z = 100 cm, which never intersects the detector.
    All Compton/photo electrons are therefore born in ``world_vol`` only.
    (An isotropic source close to the origin would instead irradiate the
    detector and create electrons in ``detector_vol``.)"""
    return f"""
/random/setSeeds {seed} {seed}
/RMG/Geometry/RegisterDetector Germanium detector_phys 0
/RMG/Output/ActivateOutputScheme Track
...
/RMG/Staging/Electrons/MaxEnergyThresholdForStacking -1 MeV
/RMG/Staging/Electrons/AddVolumeName {stack_volume}
...
def test_secondary_electron_respects_volume_selection():
    """Secondary electrons are staged only inside the configured staging volume.

    Secondaries are produced in ``world_vol``. Naming ``world_vol`` must defer
    them; naming a different volume (``detector_vol``) must leave them in stage 0.
    """
    events = scaled_events(250)

    matched_output = "stacking-secondary-volume-matched.lh5"
    unmatched_output = "stacking-secondary-volume-unmatched.lh5"

    run_macro(
        _gamma_macro_volume(seed=6000, events=events, stack_volume="world_vol"),
        matched_output,
    )
    run_macro(
        _gamma_macro_volume(seed=6000, events=events, stack_volume="detector_vol"),
        unmatched_output,
    )

    tracks_matched = read_tracks(matched_output)
    tracks_unmatched = read_tracks(unmatched_output)

    assert tracks_matched is not None, "Missing matched-volume track output"
    assert tracks_unmatched is not None, "Missing unmatched-volume track output"

    secondary_stage1_matched = count_tracks_by_stage(
        tracks_matched,
        pdg=11,
        stage=1,
        primary_only=False,
    )
    secondary_stage1_unmatched = count_tracks_by_stage(
        tracks_unmatched,
        pdg=11,
        stage=1,
        primary_only=False,
    ...
    assert secondary_stage1_matched > 0
    assert secondary_stage1_unmatched == 0
