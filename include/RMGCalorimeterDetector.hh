...
/** @brief Class to describe the calorimeter detector, mainly handles processing of the detected
...
class RMGCalorimeterDetector : public G4VSensitiveDetector {
  ...
  private:
    ...
    // one accumulator hit per detector uid, valid for the current event only.
    std::map<int, RMGDetectorHit*> fDetectorHits;
};
