 ...
 *
 *  @details This output scheme provides a purely calorimetric readout: it records a single
 *  @c RMGDetectorHit per detector and event, holding the total energy deposited in the detector
 *  (summed over all steps) and the global time of the first deposit. The following properties are
 *  recorded:
 *  - event index,
 *  - time,
 *  - energy deposition.
...
class RMGCalorimeterOutputScheme : public RMGVOutputScheme {
  ...
  public:
     ...
     * @details Only hits with a non-zero accumulated energy deposit are stored.
     */
