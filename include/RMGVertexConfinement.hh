...
class RMGVertexConfinement : public RMGVVertexGenerator {
  ...
  public:
     ...
     * the global rotation and translation are applied. The sampled depth is clamped to the
     * distance from the surface point to the far boundary along the inward normal (via
     * @c G4VSolid::DistanceToOut), so the displaced vertex can never cross to the other side
     * of the solid or leave it. For thin volumes this means an over-large sampled depth is
     * silently reduced; choose parameters consistent with the local thickness to keep the
     * resulting depth distribution meaningful.
     */
    struct DepthProfile {
        ...
        /**
         * @brief Validate the configured parameters, aborting with a fatal error on bad input.
         *
         * @details Enforces the invariants each distribution needs for @ref Sample to be
         * well-defined: @c mean > 0 for @c kExponential; @c range_hi > range_lo >= 0 for
         * @c kUniform; and additionally @c sigma > 0 for @c kTruncatedGaussian. Should be
         * called once, before sampling starts. @c kNone always validates.
         */
        void Validate() const;
    };
    ...
    struct SampleableObject {
        ...
        /**
         * @brief Displace a surface point inward according to @ref depth_profile.
         *
         * @details Samples a depth from @ref depth_profile and moves @c local_vertex inward
         * along the outward surface normal of @c solid. The depth is clamped to the distance
         * to the far boundary (@c G4VSolid::DistanceToOut) so the vertex stays inside the
         * solid. Does nothing when the profile type is @c DepthProfile::Type::kNone.
         *
         * @param local_vertex The surface point, in the local frame of @c solid. Modified in place.
         * @param solid The solid whose surface @c local_vertex lies on.
         */
