...
std::vector<std::string> getPMTNames(std::string filename)
{
  ...
  std::string key = "physvol name=\"PMT"; // The physical volume names have this as indicator before them
  if (!gdmlfile) 
  {
  ...
  while (std::getline(gdmlfile, line))
  {
    ...
    if(pos != std::string::npos)
    {
      ...
      std::string name = "PMT" + line.substr(0, pos); //Deleted the "PMT" out of the name previously so add it again
