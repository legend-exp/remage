...
```json
{
  "argv": [
   ...
   "exec",
   "-B", "/run/user/1000:/run/user/1000",
   "-B", "/home/user/scripts/remage_kernel_setup.sh:/opt/remage/remage_kernel_setup.sh",
   "/path/to/remage_base_latest.sif",
...
ge_s = pg4.geant4.solid.Tubs("germanium", pRMin=0, pRMax=100, pDz=50, pSPhi=0, pDPhi=2 * np.pi, registry=reg, lunit="mm")
...
if subprocess.run(f"remage {macro_path} -g {gdml_path} -o {output_directory}/out.lh5 -w -t 1  ", shell=True,) not in {0,2}:
...
hist.new.Reg(440, 0, 2200, name="energy [keV]").Double().fill(ak.sum(evt.edep, axis=-1)).plot(yerr=False)
