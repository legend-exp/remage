...
[macro command](../rmg-commands.md#rmgoutputactivateoutputscheme):
...
The _Germanium_ output scheme handles the output from _Germanium_ (HPGe) detectors, but would
...
to 32-bit with the macro commands (docs [link](project:../rmg-commands.md#rmgoutputgermaniumstoresingleprecisionposition),
[link](project:../rmg-commands.md#rmgoutputgermaniumstoresingleprecisionenergy)):
...
It is possible to also store the track id (see [link](https://geant4-userdoc.web.cern.ch/UsersGuides/ForApplicationDeveloper/html/TrackingAndPhysics/tracking.html) for details) and parent track id of each step with
...
or across all registered sensitive _Germanium_ detectors (if this macro command is not used).
The event is then discarded if the energy is less than or equal to `ELOW` or less than `EHIGH`.

:::{note}
This mechanism will remove the data from the event across all output schemes, not only the _Germanium_!
:::
...
with the [command](project:../rmg-commands.md#rmgoutputgermaniumdiscardphotonsifnogermaniumedep):
...
be controlled with the macro [command](project:../rmg-commands.md#rmgoutputgermaniumsteppositionmode):
...
this behaviour there is a macro [command](project:../rmg-commands.md#rmgoutputgermaniumdiscardzeroenergyhits):
...
Most functionality is similar to the _Germanium_ output scheme with a few exceptions.
...
- The velocity of the particles can be saved using the
  [command](project:../rmg-commands.md#rmgoutputscintillatorstoreparticlevelocities):
...
produced by interactions in germanium are 1-2 mm, so we are not sensitive to tracking at the micrometer level.
To reduce the file size while retaining the useful information for computing observables of interest
we have implemented some "pre-clustering" routines. These routines combine together steps that are
very close together.
...
This takes in the pointer to the original {cpp:type}`RMGDetectorHitsCollection` returning a pointer
...
of the germanium (or visa versa).

These options provide a sophisticated mechanism for handling the surface of germanium detectors!
